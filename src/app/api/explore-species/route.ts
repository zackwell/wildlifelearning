...
export async function POST(req: Request) {
  try {
    ...
    const imageTaxonHint = anchor?.taxonBrief ?? null;
...
- name 须与用户输入「${query}」完全一致`;
    ...
    const anchorMismatch =
      ...
      detectFictionalSpeciesDenial(payload, anchorForPrompt) ??
      ...
      try {
        ...
        if (retryText) {
          ...
          if (
            ...
            !detectFictionalSpeciesDenial(retryPayload, anchorForPrompt) &&
